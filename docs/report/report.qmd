...
When comparing between logistic regression, SVM with linear kernel and the SVM with the RBF kernel, we were able to replicate the original results on the TRANSFER transactions with recall scores attaining close to 99% for all three models and the SVM with RBF kernel yielding the highest AUPRC score. With the added decision tree, we also attain a recall score of around 99% but with a lower precision relative to the other three models. However, we observe the highest AUPRC score beating the SVM with RBF kernel.
